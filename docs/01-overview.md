...
본 문서에서는 주요 이슈만 요약하고, 상세 원인 분석과 해결 과정은 `docs/05-troubleshooting.md`에서 정리합니다.

| 문제                      | 원인                                                       | 해결 방향                                     | 상태    | 상세                           |
| ----------------------- | -------------------------------------------------------- | ----------------------------------------- | ----- | ---------------------------- |
| awsvpc ENI 부족           | Task별 ENI가 필요하여 단일 EC2 환경에서 다수 Task 실행 및 scale-out 제한 발생 | WS 등 일부 서비스를 bridge mode로 전환              | 해결    | `추후 파일 개별 링크로 변경` |
| Health Check 404        | ALB Health Check path와 서비스 응답 경로 불일치                     | Health Check 경로 및 서비스 응답 처리 조정            | 해결    | `추후 파일 개별 링크로 변경` |
| NLB Multi-AZ timeout    | NLB가 여러 AZ IP로 응답하지만 특정 AZ에 healthy target이 없는 상황 발생     | Target 상태, AZ 배치, NLB 연결 구조 분석            | 분석 완료 | `추후 파일 개별 링크로 변경` |
| Config mount 누락         | ECS Task 실행 시 외부 설정 파일이 컨테이너에 마운트되지 않음                   | volume 및 mountPoint 설정 추가                 | 해결    | `추후 파일 개별 링크로 변경` |
| Target unhealthy        | Security Group 또는 Target Group 설정 누락으로 Health Check 실패   | 서비스 포트, inbound rule, Target Group 설정 조정  | 해결    | `추후 파일 개별 링크로 변경` |
| Service Discovery 호환 문제 | Cloud Map SRV Record 방식과 레거시 서비스의 host 기반 접근 방식 불일치      | DS를 awsvpc mode로 구성하고 A Record 기반 접근으로 조정 | 해결    | `추후 파일 개별 링크로 변경` |
