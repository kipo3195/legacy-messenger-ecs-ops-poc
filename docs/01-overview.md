...
| 검증 항목 | 검증 내용 | 결과 | 증적 |
|---|---|---|---|
| ECS Service 상태 | 주요 서비스의 desired/running count 정상화 확인 | 성공 | `evidence/01-final-state/ecs-services-list.png`, `evidence/03-operation-scripts/describe-service-ws-result.txt` |
| Target Group 상태 | ALB/NLB Target Group healthy 상태 확인 | 성공 | `evidence/02-load-balancer/alb-ws-target-healthy.png`, `evidence/02-load-balancer/tg-ds-33001-healthy.png` |
| 외부 접근 | NLB를 통한 DS 33001 TCP 연결 확인 | 성공 | `evidence/03-operation-scripts/test-port-ds-33001.txt` |
| 내부 연결 | Cloud Map DNS 기반 DS 내부 연결 확인 | 성공 | `evidence/04-troubleshooting/4.2-cloudmap-srv-compatibility/02-cloudmap-ds-nslookup-success.txt`, `evidence/04-troubleshooting/4.2-cloudmap-srv-compatibility/03-cloudmap-ds-port-connectivity-success.txt` |
| 확장성 | WS 서비스 scale-out 동작 확인 | 성공 | `evidence/03-operation-scripts/scale-service-ws-1-to-2.txt`, `evidence/03-operation-scripts/check-target-health-ws.txt` |

## 8. 주요 문제 해결 요약

ECS 전환 과정에서 서비스 실행, 로드밸런서 연동, 내부 DNS 연결, Health Check 구성 과정에서 여러 운영 이슈가 발생했습니다.

본 문서에서는 아키텍처 결정과 운영 검증에 직접적인 영향을 준 주요 이슈만 요약하고, 상세 원인 분석과 해결 과정은 [`docs/05-troubleshooting.md`](./05-troubleshooting.md)에서 정리합니다.

| 문제                                      | 원인                                                                                           | 해결 방향                                                                                                  | 상태    |
| --------------------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ----- |
| awsvpc ENI 부족과 Network Mode 재설계         | 모든 서비스를 awsvpc mode로 구성할 경우 Task별 ENI가 필요하여 제한된 EC2 환경에서 다수 서비스 실행과 scale-out에 제약 발생         | Websocket, Certify, Notificator, Presence, Fetch, File은 bridge mode로 구성하고, Dispatcher는 awsvpc mode로 분리 | 해결    |
...
| NLB Multi-AZ timeout                    | NLB DNS가 여러 AZ IP로 응답하지만, 특정 AZ에 healthy target이 없는 경우 일부 IP 접근 시 timeout 발생                 | Target Group health, AZ별 target 배치 상태, NLB 연결 구조를 분석하여 원인 확인                                           | 분석 완료 |
| Target Group unhealthy와 Health Check 조정 | Health Check path 불일치, Security Group 인바운드 누락, Target Group 포트 설정 오류 등으로 Target unhealthy 발생 | Health Check 경로, 서비스 응답, Target Group 포트, Security Group 규칙을 조정                                        | 해결    |


## 9. 향후 확장 계획

현재는 ECS 기반 운영 환경 구성과 기본 기능 검증까지 완료된 상태입니다.

이후에는 배포 자동화, 운영 관측, 서비스 제어 API를 추가하여 ECS 운영 포트폴리오를 확장할 계획입니다.

| 항목                    | 목적                                                    | 상태      |
| --------------------- | ----------------------------------------------------- | ------- |
| Go Controller Service | ECS Service 상태 조회, start/stop, scale, redeploy API 제공 | Planned |
...
위 항목들은 현재 POC의 완료 범위에는 포함하지 않았으며, ECS 운영 자동화와 관측 체계를 확장하기 위한 후속 작업으로 구분합니다.
