...
| 검증 항목 | 검증 내용 | 결과 | 증적 |
|---|---|---|---|
| ECS Service 상태 | 주요 서비스의 desired/running count 정상화 확인 | 성공 | `evidence/screenshots/ecs-service-running.png` |
| Target Group 상태 | ALB/NLB Target Group healthy 상태 확인 | 성공 | `evidence/screenshots/target-group-healthy.png` |
| 외부 접근 | ALB/NLB를 통한 외부 연결 확인 | 성공 | `evidence/test-results/connectivity-test.md` |
| 내부 연결 | Cloud Map DNS 기반 서비스 간 내부 연결 확인 | 성공 | `evidence/test-results/cloud-map-dns-test.md` |
| 클라이언트 기능 | 기존 클라이언트 로그인 성공 확인 | 성공 | `evidence/screenshots/client-login-success.png` |
| 메신저 기능 | 채팅, 쪽지 등 기본 기능 동작 확인 | 성공 | `evidence/test-results/messenger-feature-test.md` |
| 확장성 | WS 서비스 scale-out/scale-in 동작 확인 | 성공 | `evidence/test-results/ws-scale-test.md` |

## 8. 주요 문제 해결 요약

| 문제 | 원인 | 해결 방향 | 상태 | 상세 |
|---|---|---|---|---|
| awsvpc ENI 부족 | Task별 ENI 필요로 단일 EC2에서 확장 제한 | WS 서비스를 bridge mode로 전환 | 해결 | `troubleshooting/01-eni-resource-limit.md` |
| Health Check 404 | ALB Health Check path와 서비스 응답 불일치 | Health Check 경로 및 응답 처리 조정 | 해결 | `troubleshooting/02-health-check-404.md` |
| NLB Multi-AZ timeout | AZ별 healthy target 불균형 가능성 | Target 상태와 AZ 구조 분석 | 분석 완료 | `troubleshooting/03-nlb-multi-az-timeout.md` |
| Config mount 누락 | ECS 실행 시 외부 설정 파일 미탑재 | volume/mountPoint 구성 | 해결 | `troubleshooting/04-config-mount-missing.md` |
| Target unhealthy | Security Group 포트 미허용 | inbound rule 정리 | 해결 | `troubleshooting/05-security-group-target-unhealthy.md` |
